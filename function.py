...
def createaccount(data,pin,bal):
    ...
    balance=int(bal)
    password=data['cpass']
    supabase.table('mydata').insert({"username": username,"pin": pin,"firstname": fname, "lastname": lname,"email": email,"balance":balance,"password":password}).execute()
...
def login(data):
    ...
    if(suparesponse):
        temp=suparesponse[0]
        if(username==temp['username'] and password==temp['password']):
            return 3
        elif(email==temp['email'] and password==temp['password']):
            return 3
        else:
            return 2
    else:
